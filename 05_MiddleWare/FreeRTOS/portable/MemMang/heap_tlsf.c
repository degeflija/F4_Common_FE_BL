...
tlsf_t * __attribute__ ((section ("user_data"))) second_tlsf;
...
void vPortInitMemory(void)
{
	...
	#endif
#if USE_SECOND_HEAP
	  second_tlsf = tlsf_create_with_pool( &__FreeRTOS_second_heap_begin__, &__FreeRTOS_second_heap_end__ - &__FreeRTOS_second_heap_begin__);
	#if DUMP
	  trace_printf ("Memory Pool: 0x%08X-0x%08X\n", &__FreeRTOS_second_heap_begin__, &__FreeRTOS_second_heap_end__);
	#endif
#endif
}
...
#if USE_SECOND_HEAP
   if( res == 0)
	   res = tlsf_malloc( second_tlsf, xWantedSize);
#endif
   xTaskResumeAll();
#if DUMP
...
void * pvPortMallocAlignedMemory(size_t xWantedSize, size_t alignment)
{
   ...
   if( res == 0)
	   res = tlsf_memalign( second_tlsf, alignment, xWantedSize);
   xTaskResumeAll();
#if DUMP
