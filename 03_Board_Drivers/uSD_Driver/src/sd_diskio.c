...
extern QueueHandle_t SDIO_LTDC_mutex;
 ...
 */
#define SD_TIMEOUT 1000 // used to be 30 * 1000
...
DRESULT SD_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
  ...
  */
  ...
  {
#endif
	xSemaphoreTake( SDIO_LTDC_mutex, 0xffffffff);
    ...
    if (event.status == osEventMessage)
    {
      if (event.value.v == READ_CPLT_MSG)
      {
        ...
        while(osKernelSysTick() - timer <SD_TIMEOUT)
          ...
          xSemaphoreGive( SDIO_LTDC_mutex);
    ...
    else
    {
      ...
      for (i = 0; i < count; i++)
      {
        uint32_t ret = BSP_SD_ReadBlocks_DMA((uint32_t*)scratch, (uint32_t)sector++, 1);
        if (ret == MSD_OK )
        {
